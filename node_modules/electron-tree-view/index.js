...
module.exports = function (opts) {
  ...
  const renderItem = opts.renderItem || ((hx, data, children, loadHook, clickElem, createChild) => {
    if(data.type == 'directory'){
      ...
      var icon = 'ic_file.png';

      try {
        if(loadedDeviceIndex == false && ( data.name == 'index.php' || data.name == 'index.html')){
          

          /*var cssLink = document.createElement('link');
          cssLink.href = '../lib/ui/base.css';
          cssLink.rel = 'stylesheet';
          cssLink.type = 'text/css';
          var frm = frames['mobile_device'].document.head.appendChild(cssLink);*/
          loadedDeviceIndex = true;

          var frame = document.getElementById('mobile_device');
          //frame.src = data.path.replace('\\','/');
          //console.log([frame])
        }
      } catch (e) {
      ...
      //icon = data.extension === null || data.extension === '' || data.extension === 'undefined' ? 'todo.svg' : data.extension.substring(1, data.extension.length) + '.svg';


      icon = 'file.svg';
      if ((data.extension === null || data.extension === '' || data.extension === 'undefined') === false) {
        if (fs.existsSync(__dirname + '/icons/' + data.extension.substring(1, data.extension.length) + '.svg')){
            icon = data.extension.substring(1, data.extension.length) + '.svg';
        }else{
          switch (data.extension.substring(1, data.extension.length)) {
            case 'js':
                icon = 'javascript.svg'
              break;
            case 'ico':
      ...
      <a href="#" class="header" onclick=${clickElem}>
        <div>
        ${hx`<img src="${__dirname + '/icons/'+icon}" class="f-icon" />`}
         <span>${opts.label ? opts.label(data) : data.name}</span>
        </div>
      </a>
    </div>`
